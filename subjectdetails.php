...
if (isset($_GET['sid'])) {
    $sid = $_GET['sid'];
    $sqlsubject = "SELECT tbl_subjects.tutor_id, tbl_tutors.tutor_name, tbl_subjects.subject_id, tbl_subjects.subject_name, tbl_subjects.subject_description, tbl_subjects.subject_price, tbl_subjects.subject_sessions, tbl_subjects.subject_rating FROM tbl_subjects INNER JOIN tbl_tutors ON tbl_subjects.tutor_id = tbl_tutors.tutor_id WHERE subject_id = '$sid'";
    ...
    <body>
        ...
        <div class="container">
            ...
            foreach ($rows as $subjects) {
                $i++;
                $sid = $subjects['subject_id'];
                $tid = $subjects['tutor_id'];
                $sname = $subjects['subject_name'];
                $stname = $subjects['tutor_name'];
                $sdesc = $subjects['subject_description'];
                $sprice = number_format((float)$subjects['subject_price'], 2, '.', '');
                $ssession = $subjects['subject_sessions'];
                $srate = $subjects['subject_rating'];
                echo "<div class='w3-row w3-border w3-card-4 w3-round' style='margin:10px'>
                <div class='w3-container w3-half'><br>
                    <img class='w3-image' src=assets/courses/$sid.png" . " onerror=this.onerror=null;this.src='res/avatar.jpg'"
                    . " style='width:100%;'><br><br>
                </div>
                <div class='w3-container w3-half'>
                    <div class='w3-padding'><p><h3><b>$sname</b></h3>Taught by <a href='tutortdetails.php?tid=$tid'>$stname</a><br><br><b>Description</b><br>$sdesc<br><br><b>Price:</b> RM$sprice<br><b>Session:</b> $ssession<br><b>Rating:</b> $srate</div>
                    <div class='w3-button w3-purple w3-round w3-right' style='width:auto'><a href='menu.php' class='w3-button'>Back</a></div></p></div>
                </div></div>";
            }
            ?>
        </div>
